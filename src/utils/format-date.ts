...
export const formatDate = (str: string) => {
  ...
  if (diffDays === 0) {
    return `Сегодня в ${addZeroBefore(past.getHours())}:${addZeroBefore(past.getMinutes())}`;
  }

  if (diffDays === 1) {
    return `Вчера в ${addZeroBefore(past.getHours())}:${addZeroBefore(past.getMinutes())}`;
  }

  if (diffDays === 2) {
    return `Позавчера в ${addZeroBefore(past.getHours())}:${addZeroBefore(past.getMinutes())}`;
  }

  if (diffDays > 2 && diffDays < 30) {
    return `${diffDays} назад`;
  }
